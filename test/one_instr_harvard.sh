...
SIM_OUTPUT=$(test/2-simulator/mips_cpu_harvard_tb_${TESTCASE})
# Capture the exit code of the simulator in a variable
RESULT=$?
echo "${SIM_OUTPUT}" > test/3-output/${INSTR}/mips_cpu_harvard_tb_${TESTCASE}.stdout
set -e
...
set +e
COMPARE=$(diff -w test/4-reference/${INSTR}/${TESTCASE}.out test/3-output/${INSTR}/mips_cpu_harvard_tb_${TESTCASE}.out)
RESULT=$?
set -e
